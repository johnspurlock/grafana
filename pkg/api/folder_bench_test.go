...
func BenchmarkFolderService_Search(b *testing.B) {
	m, orgID, userWithPermissions := setupBench(b)
	all := LEVEL0_FOLDER_NUM * LEVEL1_FOLDER_NUM * LEVEL2_FOLDER_NUM * LEAF_DASHBOARD_NUM
	req := httptest.NewRequest(http.MethodGet, fmt.Sprintf("/api/search?type=dash-db&limit=%d", all), nil)
	req = webtest.RequestWithSignedInUser(req, &user.SignedInUser{UserID: userWithPermissions, OrgID: orgID})
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		rec := httptest.NewRecorder()
...
func setupBench(b testing.TB) (*web.Macaron, int64, int64) {
	...
	require.NoError(b, err)
	userSvc, err := userimpl.ProvideService(db, orgService, cfg, teamSvc, localcache.ProvideService(), &quotatest.FakeQuotaService{}, bundleregistry.ProvideService())
	require.NoError(b, err)
	...
	folderServiceWithFlagOn := folderimpl.ProvideService(acimpl.ProvideAccessControl(cfg), bus.ProvideBus(tracing.InitializeTracerForTest()), cfg, dashStore, folderStore, db, featuresFlagOn)
	...
	now := time.Now()
	teamSrv := teamimpl.ProvideService(db, cfg)
	roleIDs := make([]int64, 0, TEAM_NUM)
	roles := make([]accesscontrol.Role, 0, TEAM_NUM)
	teamRoles := make([]accesscontrol.TeamRole, 0, TEAM_NUM)
	for i := 0; i <= TEAM_NUM; i++ {
		t, err := teamSrv.CreateTeam(fmt.Sprintf("team%d", i), "", orgID)
		require.NoError(b, err)
		require.NotZero(b, t.ID)
		signedInUser.Teams = append(signedInUser.Teams, t.ID)

		for _, userID := range userIDs {
			teamSrv.AddTeamMember(userID, orgID, t.ID, false, dashboards.PERMISSION_VIEW)
		}
	...
	permissions := make([]accesscontrol.Permission, 0, len(roleIDs))
	var roleID int64 = 1
	...
	folderPermissions, err := ossaccesscontrol.ProvideFolderPermissions(
		cfg, routing.NewRouteRegister(), db, ac, license, &dashboards.FakeDashboardStore{}, folderServiceWithFlagOn, acSvc, teamSrv, userSvc)
	require.NoError(b, err)
	...
	dashboardSvc, err := dashboardservice.ProvideDashboardServiceImpl(
		...
		featuresFlagOn, folderPermissions, dashboardPermissions, acimpl.ProvideAccessControl(cfg),
